// Family Vacation Planner V2.2.17 — destination-aware specialist module
...
function openNewTripDialog(){const d=$('#newTripDialog');if(!d)return;populateDestinationSelects();$('#newTripStartPanel')?.classList.remove('hidden');$('#destinationFinderPanel')?.classList.add('hidden');$('#destinationFinderResults').innerHTML='';destinationFinderRun=0;applyTranslations();d.classList.remove('hidden');}
...
function scoreDestinations(){
  const moods=finderSelections(),budget=$('#finderBudget')?.value||'medium',length=$('#finderLength')?.value||'week',climate=$('#finderClimate')?.value||'any',setting=$('#finderSetting')?.value||'any';
  if(!moods.length)return [];
  const targetCost={low:1,medium:2,high:3}[budget];
  return Object.entries(destinationFinderProfiles).map(([key,d])=>{
    let score=(d.length[length]||3)*4-climateFit(d,climate)*-1+settingFit(d,setting);
    const matched=[];let weakest=5;
    for(const mood of moods){const v=d.tags[mood]||0;weakest=Math.min(weakest,v);score+=v*v*2;if(v>=4)matched.push(mood);if(v<=1)score-=22;else if(v===2)score-=7;}
    score-=Math.abs(d.cost-targetCost)*5;
    // A multi-priority search should favour destinations that satisfy the whole brief,
    // not one superstar trait and two weak ones.
    if(moods.length>1)score+=weakest*6;
    const preset=presetFor(key);return {key,name:preset?.name||key,score,reason:d.reason,matched,climate:d.climate||[],setting:d.setting||[]};
...
function renderDestinationFinder(forceDifferent=false){
  ...
  if(!all.length){root.innerHTML=`<div class="finder-empty">${escapeHtml(t('finderNeed','Choose at least one priority to get started.'))}</div>`;if($('#findDestinations'))$('#findDestinations').textContent=t('showIdeas','Show me ideas');return;}
  if(forceDifferent)destinationFinderRun++;const pageSize=5,maxStart=Math.max(0,all.length-pageSize);const start=Math.min((destinationFinderRun*pageSize)%Math.max(pageSize,all.length),maxStart);let results=all.slice(start,start+pageSize);if(results.length<pageSize)results=[...results,...all.slice(0,pageSize-results.length)];
  const topScore=all[0]?.score||1,bottomScore=all[Math.min(12,all.length-1)]?.score||0,range=Math.max(1,topScore-bottomScore);
  root.innerHTML=results.map((r,i)=>{const pct=Math.max(55,Math.min(98,Math.round(72+((r.score-bottomScore)/range)*26)));const match=r.matched.length?r.matched.map(finderMoodLabel).join(' · '):finderSelections().map(finderMoodLabel).join(' · ');return `<article class="destination-result"><div class="destination-rank">${start+i+1}</div><div><div class="destination-title-row"><h3>${escapeHtml(r.name)}</h3><span class="destination-match">${pct}% fit</span></div><p>${escapeHtml(r.reason)}</p><div class="destination-fit">Best match: ${escapeHtml(match)}</div></div><button class="small-btn primary-small choose-destination" data-destination="${r.key}" type="button">${t('planThis')}</button></article>`;}).join('');
