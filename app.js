// Family Vacation Planner V2.2.18 — destination finder travel-cost guardrails
...
function renderDestinationFinder(forceDifferent=false){
  ...
  if(!all.length){root.innerHTML=`<div class="finder-empty">No strong matches meet that brief yet. Try loosening the budget, trip length or one priority.</div>`;if($('#findDestinations'))$('#findDestinations').textContent=t('showIdeas','Show me ideas');return;}
  ...
  root.innerHTML=results.map((r,i)=>{
    ...
    const trade=r.tradeoffs?.length?`<div class="destination-tradeoff">Trade-off: ${escapeHtml(r.tradeoffs.join(' · '))}</div>`:'';
    return `<article class="destination-result"><div class="destination-rank">${start+i+1}</div><div><div class="destination-title-row"><h3>${escapeHtml(r.name)}</h3><span class="destination-match">${pct}% fit</span></div><p>${escapeHtml(r.reason)}</p><div class="destination-fit">Why it fits: ${escapeHtml(fit)}</div>${trade}</div><button class="small-btn primary-small choose-destination" data-destination="${r.key}" type="button">${t('planThis')}</button></article>`;
  }).join('');
  if(pool.length<=pageSize)root.insertAdjacentHTML('beforeend',`<div class="finder-empty">These are the strongest matches for this brief. Change a priority or loosen a constraint for more ideas.</div>`);
  else if(start+pageSize>=pool.length)root.insertAdjacentHTML('beforeend',`<div class="finder-empty">That’s the end of the strong-match pool. The next re-run starts again rather than dropping into poor fits.</div>`);
