// Vacation Planner V2.6.6 — stable Orlando startup + onboarding layer
(()=>{
  'use strict';

  if(window.__VP_ORLANDO_BOOTSTRAP_266__)return;
  window.__VP_ORLANDO_BOOTSTRAP_266__=true;

  const VERSION='2.6.6';
  const RELEASE_KEY='vp_orlando_demo_release'; // intentionally NOT ffvp_* so app-data resets cannot delete it
  const ONBOARDING_METRICS_KEY='ffvp_orlando_onboarding_metrics';
  const ORLANDO_ZONE='America/New_York';
  const SPLASH_MIN_MS=1250;

  let onboardingStart=0,stepStart=0,lastStep=-1;
  ...
  function injectSplash(){
    ...
    style.textContent=`
      #vpStartupSplash{position:fixed;inset:0;z-index:100000;display:grid;place-items:center;padding:20px;background:linear-gradient(155deg,#f8f9fa 0%,#eefafa 45%,#fff3ec 100%);font-family:Inter,system-ui,sans-serif;color:#0f172a;overflow:hidden}
      ...
      .vp-startup-card{position:relative;z-index:1;width:min(100%,390px);text-align:center;padding:27px 22px 22px;border:1px solid rgba(255,255,255,.9);border-radius:30px;background:rgba(255,255,255,.86);box-shadow:0 24px 70px rgba(15,23,42,.10)}
      .vp-startup-logo{width:min(255px,78%);height:auto;display:block;margin:0 auto 14px}.vp-startup-kicker{font-size:10px;font-weight:900;letter-spacing:.13em;color:#0f817b}.vp-startup-title{font-family:"Plus Jakarta Sans",Inter,sans-serif;font-size:23px;line-height:1.17;margin:9px 0 8px;color:#152b34}.vp-startup-copy{font-size:13px;line-height:1.5;color:#607279;margin:0 auto 19px;max-width:310px}
      .vp-startup-status{font-size:13px;font-weight:800;color:#29464d;min-height:20px}.vp-startup-detail{font-size:11px;color:#78898e;margin-top:7px;min-height:17px}.vp-startup-track{height:5px;margin-top:14px;border-radius:999px;background:#e2eceb;overflow:hidden}.vp-startup-bar{display:block;width:12%;height:100%;border-radius:inherit;background:linear-gradient(90deg,#19a69d,#ff6b6b);transition:width .28s ease}.vp-startup-dots{display:flex;justify-content:center;gap:6px;margin-top:13px}.vp-startup-dots i{width:6px;height:6px;border-radius:50%;background:#9bcfca;animation:vpStartupPulse 1s ease-in-out infinite}.vp-startup-dots i:nth-child(2){animation-delay:.16s}.vp-startup-dots i:nth-child(3){animation-delay:.32s}
      #vpStartupSplash.vp-startup-leaving{opacity:0;transition:opacity .22s ease;pointer-events:none}@keyframes vpStartupPulse{0%,100%{opacity:.35;transform:scale(.82)}50%{opacity:1;transform:scale(1.15)}}
      @media(max-width:420px){#vpStartupSplash{padding:16px}.vp-startup-card{padding:23px 17px 20px;border-radius:25px}.vp-startup-title{font-size:20px}.vp-startup-copy{font-size:12.5px}}
  ...
  function splashProgress(p,status,detail){
    const bar=qs('#vpStartupBar');if(bar)bar.style.width=`${Math.max(8,Math.min(100,p))}%`;
  ...
  function resetRuntimeState(){
    try{
      if(typeof state!=='undefined'){
        state.profile={familyName:'',destinationPreset:'orlando',homeBase:'',arrivalDate:'',departureDate:'',maxDrive:30,budget:'medium',notes:'',quickNotes:[],members:[]};
        state.saved=[];state.tripStatuses={};state.plans=[];state.discovered={};state.recommendationFeedback={};state.decisionEvents=[];
      }
    }catch{}
  }

  function resetForNewReleaseOnce(){
    let previous='';try{previous=localStorage.getItem(RELEASE_KEY)||'';}catch{}
    const changed=previous!==VERSION;
    if(changed){
      splashProgress(22,'New demo version ready','Starting with a clean Orlando trip');
      try{
        const keys=[];
        for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith('ffvp_'))keys.push(k);}
        keys.forEach(k=>localStorage.removeItem(k));
        localStorage.setItem(RELEASE_KEY,VERSION);
      }catch{}
      resetRuntimeState();
    }
    // Prevent the old beta shell from independently forcing landing/onboarding on later launches.
    try{
  ...
  function loadStyle(href,id){if(document.getElementById(id))return;const link=document.createElement('link');link.id=id;link.rel='stylesheet';link.href=href;document.head.appendChild(link);}
  function loadScript(src,id,timeout=1800){
    return new Promise(resolve=>{
      if(document.getElementById(id)){resolve(true);return;}
      let done=false;const finish=ok=>{if(done)return;done=true;clearTimeout(timer);resolve(ok);};
  ...
  function writeProfilePatch(patch){try{const p={...readProfile(),...patch,destinationPreset:'orlando'};localStorage.setItem('ffvp_profile',JSON.stringify(p));if(typeof state!=='undefined'&&state?.profile)Object.assign(state.profile,p);}catch{}}
  ...
  function markOrlandoOnly(){
    document.body.classList.add('orlando-early-access');setText(qs('.brand-kicker'),'ORLANDO EARLY ACCESS · V2.6.6');
    ...
    qs('#newTripDialog')?.style.setProperty('display','none');qs('#orlandoLandingCopy')?.remove();
  }
  ...
  function rewriteOnboarding(){
    ...
    if(hero){setText(qs(':scope > p',hero),'A few useful details now means less thinking once you’re in Orlando. I’ll remember the people, the pace and the practical bits for the whole trip.');if(!qs('.orlando-onboarding-badge',hero)){const badge=document.createElement('div');badge.className='orlando-onboarding-badge';badge.textContent='ORLANDO FAMILY HOLIDAY SETUP';qs('.setup-progress-wrap',hero)?.before(badge);}}
    const steps=qsa('.setup-step',onboarding);
    if(steps[0]){
      setText(qs('h3',steps[0]),'Tell me about your Orlando trip');setText(qs('.step-copy',steps[0]),'Just enough to know when you’re there and where the family is starting from.');
      ...
      const a=qs('#setupArrivalDate'),d=qs('#setupDepartureDate');if(a)a.required=true;if(d)d.required=true;
      setText(qs('.field-help',steps[0]),'Your dates tell me whether we’re planning ahead, deciding what to do today, or running out of holiday days.');setText(qs('.setup-next',steps[0]),'Who’s coming? →');
    ...
    if(steps[2]){setText(qs('h3',steps[2]),'How do you want Orlando to feel?');setText(qs('.step-copy',steps[2]),'Give me the rough pace, travel range and spending mood. We can learn the rest as you use the trip.');injectPaceControl(steps[2]);setHTML(qs('.launch-card',steps[2]),'<span>✨</span><div><b>That’s enough to start being useful.</b><small>Add bookings, must-dos and extra preferences later.</small></div>');setText(qs('button[type="submit"]',steps[2]),'Show me what makes sense ✦');}
    qs('#skipSetup')?.style.setProperty('display','none');compactCrewRows();
  ...
  function revealApp(releaseChanged){
    const onboarded=!!localStorage.getItem('ffvp_onboarded');
    qs('#landingScreen')?.classList.add('hidden');
    if(!onboarded){startMetrics();try{if(typeof showOnboarding==='function')showOnboarding();else qs('#onboarding')?.classList.remove('hidden');}catch{qs('#onboarding')?.classList.remove('hidden');}}
    else qs('#onboarding')?.classList.add('hidden');
    splashProgress(100,releaseChanged?'Fresh demo ready':'Ready','');
    const elapsed=performance.now()-bootStarted;setTimeout(()=>{const splash=qs('#vpStartupSplash');if(!splash)return;splash.classList.add('vp-startup-leaving');setTimeout(()=>{splash.remove();qs('#vpStartupCriticalCss')?.remove();},240);},Math.max(0,SPLASH_MIN_MS-elapsed));
  }

  async function init(){
    injectSplash();
    const releaseChanged=resetForNewReleaseOnce();
    splashProgress(34,releaseChanged?'Starting a clean Orlando trip':'Getting your Orlando trip ready…','Preparing your trip tools');
    ...
    setTimeout(()=>splashProgress(60,'Connecting the planner…','Loading Orlando decision tools'),260);
    const basePromise=loadScript(`/base-location.js?v=${VERSION}`,'vpBaseLocationRuntime',1700);
    const demoPromise=loadScript(`/decision-demo.js?v=${VERSION}`,'vpDecisionDemoRuntime',1700);
    Promise.allSettled([basePromise,demoPromise]).then(()=>{personalizeDecisionExperience();syncOrlandoLocation();});
    loadScript('/family-ui-test.js?v=6','vpFamilyUiTestRuntime',1200).then(()=>{});

    setTimeout(()=>splashProgress(84,'Almost there…','Finishing the Orlando setup'),700);
    setTimeout(()=>revealApp(releaseChanged),1050);
